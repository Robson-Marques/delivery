...
export function SettingsPanel() {
  ...
  const saveSettings = async () => {
    ...
    const { error } = await supabase.from('establishment_settings').update({
      ...
      default_delivery_fee: parseFloat(form.default_delivery_fee),
  ...
  const addNeighborhood = async () => {
    ...
    const { error } = await supabase.from('neighborhoods').insert({
      ...
      delivery_fee: parseFloat(newNeighborhood.delivery_fee) || 5.99,
    });
  ...
  const removeNeighborhood = async (id: string) => {
    const { error } = await supabase.from('neighborhoods').update({ is_active: false }).eq('id', id);
      ...
      <section>
        ...
        <div className="bg-card rounded-xl border border-border p-4 space-y-3">
          <div className="flex gap-2">
            ...
            <button onClick={addNeighborhood} className="p-2 rounded-lg bg-primary text-primary-foreground">
          ...
          <div className="space-y-1">
            {neighborhoods.filter(n => n.is_active).map(n => (
              <div key={n.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => removeNeighborhood(n.id)} className="p-1 rounded hover:bg-destructive/10">
            ...
            {neighborhoods.filter(n => n.is_active).length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-4">Nenhum bairro cadastrado</p>
            )}
          </div>
        </div>
      </section>
